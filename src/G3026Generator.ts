...
const MARGIN_LEFT = 851; 
const MARGIN_RIGHT = 851; 
const MARGIN_HEADER = 425; 
...
// BORDER SYSTEM
// ════════════════════════════════════════════════════════════════════════════
const BORDER_THICK_VAL = 18; 
const BORDER_THIN_VAL = 12; 
...
export const generateG3026Docx = async (data: AppState['g3026'], g3022Data?: AppState['g3022']): Promise<Blob> => {
  ...
  groups.forEach((group) => {
    ...
    items.forEach((item, index) => {
        ...
        else if (item.status === ComplianceStatus.CLARIFY) statusMark = 'X';
                    ...
                    ...samplingResults.map((item, idx) => new TableRow({
                        children: [
                            ...
                            new TableCell({ borders: {top: BORDER_THIN, left: BORDER_THIN, bottom: BORDER_THIN, right: BORDER_THIN}, children: [stdP([txt(`${item.type}\n${item.ratio || ''}`)])] }), 
                             new TableCell({ borders: {top: BORDER_THIN, left: BORDER_THIN, bottom: BORDER_THIN, right: BORDER_THIN}, children: [stdP([txt(item.ratio || "")])] }), 
