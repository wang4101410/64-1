...
export const generateG3027Docx = async (data: AppState['g3027']): Promise<Blob> => {
  ...
  const findingRows = [];
  const minRows = 3; 
  const totalRows = Math.max(findings.length, minRows);

  for (let i = 0; i < totalRows; i++) {
      const item = findings[i] || { id: '', type: '', description: '', reporter: '', correctiveAction: '', reviewOpinion: '', reviewer: '', result: '', location: '' };
      ...
      findingRows.push(new TableRow({
          ...
          children: [
              new TableCell({ children: [stdP([txt(i < findings.length ? `${i + 1}` : "")], "center")], verticalAlign: VerticalAlign.CENTER, borders: { top: BORDER_THIN, bottom: BORDER_THIN, left: BORDER_THICK, right: BORDER_THIN } }),
        ...
        children: [
            new TableCell({
                ...
                children: [
                    ...
                    new Paragraph({ children: [], spacing: { before: 400, after: 400 } }), // Space for signature
  ...
  const doc = new Document({
    styles: {
      default: {
        document: {
          run: { font: FONTS_OPTS, size: 24 }, // Fixed
